...
class PokemonSortedPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Pokemon> {
        ...
        return try {
            Log.d("PokemonSortedPagingSource", "Loading page $page, limit $limit, offset $offset")

            // 1. Загружаем данные с API
            val response = apiService.getPokemonList(limit = limit, offset = offset)
            var pokemons = response.results.mapIndexed { index, result ->
                val id = offset + index + 1
                Log.d("PokemonSortedPagingSource", "Fetching details for ${result.name}, id: $id")
            ...
            // 2. Сортируем только загруженные данные
            pokemons = when (criteria) {
                "Number" -> pokemons.sortedBy { it.id }
                "Name" -> pokemons.sortedBy { it.name.lowercase() }
                "HP" -> pokemons.sortedBy { it.hp }
                "Attack" -> pokemons.sortedBy { it.attack }
                "Defence" -> pokemons.sortedBy { it.defense }
                else -> pokemons
            ...
            val sortedCached = when (criteria) {
                ...
                "Name" -> cached.sortedBy { it.name.lowercase() }
